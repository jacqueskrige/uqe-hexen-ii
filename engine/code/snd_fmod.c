...
qboolean SND_FOpen (const char *name)
{
	...
	if ((len = COM_FOpenFile((char *)name, &f, false)) < 1)
	{
...
===================
*/
void MID_Start (char *name, qboolean loop, qboolean notify)
{
#ifdef UQE_FMOD_MIDI
	MOD_Start(name, loop, notify);
#else
	MIDI_Play(name);
	MIDI_Loop((int)loop);
#endif
...
===================
*/
void MID_Resume (qboolean force)
{
#ifdef UQE_FMOD_MIDI
	MOD_Resume(force);
#else
...
void FMOD_MusicStart (char *name, qboolean loop, qboolean notify)
{
	...
	if (name != NULL && strlen(name) != 0)
	{
		...
		len = COM_FOpenFile((char *)trackname, &f, false);
		if (len < 0)
		{
		...
		if (len < 0)
		{
			sprintf(trackname, "music/%strack%02i.ogg", (hexen2_portals) ? "mp_" : "", atoi(file));
			len = COM_FOpenFile((char *)trackname, &f, false);
		}
		if (len < 0)
		{
			sprintf(trackname, "music/%strack%02i.mp3", (hexen2_portals) ? "mp_" : "", atoi(file));
			len = COM_FOpenFile((char *)trackname, &f, false);
		}
		if (len < 0)
		{
			sprintf(trackname, "music/%strack%02i.wav", (hexen2_portals) ? "mp_" : "", atoi(file));
			len = COM_FOpenFile((char *)trackname, &f, false);
		}
	...
	// check for midi playback
	if (cl.midi_name != NULL && strlen(cl.midi_name) != 0 && bgmmidi.value)
	{
		COM_StripExtension(cl.midi_name, file);

		sprintf(trackname_midi, "midi/%s.mid", file);
		len_midi = COM_FOpenFile((char *)trackname_midi, &f, false);
...
#ifdef UQE_FMOD_MIDI
			strcpy(trackname, trackname_midi);
#else
	...
	if (len > 0)
	{
		...
		FMOD_MusicUpdate((len_midi > 0 && bgmmidi.value) ? "midi" : "mod");
	}
	else
	{
	...
	if (strcmpi(bgmtype, "midi") == 0)
		MID_Start(SND_MusicChannel.trackname, loop, notify);
}
