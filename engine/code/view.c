...
// jkrige - gamma
#ifdef	GLQUAKE
cvar_t		gl_gamma = {"gamma", "2.35", true};
...
void V_CalcRefdef (void)
{
	...
	// jkrige - smoothed stair stepping
	static float steprate = 1;
	static float steprate_accum = 1;
	// jkrige - smoothed stair stepping
...
// set up gun position
	...
	for (i=0 ; i<3 ; i++)
	{
		...
		view->origin[i] += forward[i]*bob*0.12;
		// jkrige - view weapon bob fix
...
// set up the refresh position
	...
	if (cl.onground && ent->origin[2] - oldz > 0)
	{
		...
		// jkrige - smoothed stair stepping
		oldz += steptime * (80*steprate*steprate_accum);
		//oldz += steptime * 80;
		// jkrige - smoothed stair stepping
		...
		// jkrige - smoothed stair stepping
		steprate_accum += 0.05;
		steprate = 0.125*(ent->origin[2] - oldz);
		if (steprate < 1)
			steprate = 1;

		if (ent->origin[2] - oldz > 56)
