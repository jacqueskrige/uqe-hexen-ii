...
enum 
{
	...
	OPT_LOOKSTRAFE,	//12
	OPT_CROSSHAIR,	//13
	OPT_ALWAYSMLOOK,//14 jkrige - mlook cvar
	OPT_ALWAYSSLOOK,//15 jkrige - slook cvar
	OPT_VIDEO,		//17
	OPT_USEMOUSE,	//18
	OPTIONS_ITEMS
};
...
void M_AdjustSliders (int dir)
{
	...
	case OPT_MUSICMIDI: // bgm type
		Cvar_SetValue("bgmmidi", !bgmmidi.value);
		break;
...
void M_Options_Draw (void)
{
	...
	M_Print (16, 60+(OPT_DEFAULTS*8), "     Reset to defaults");

	M_Print (16, 60+(OPT_SCRSIZE*8), "           Screen size");
	// jkrige - viewsize decreased
	//r = (scr_viewsize.value - 30) / (120 - 30);
	//r = (scr_viewsize.value - 20) / (110 - 20);
	r = (scr_viewsize.value - 100) / (110 - 100);
	// jkrige - viewsize decreased
	M_DrawSlider (220, 60+(OPT_SCRSIZE*8), r);

	// jkrige - change brightness slider for opengl
	M_Print (16, 60+(OPT_GAMMA*8), "            Brightness");
#ifdef GLQUAKE
	r = (2.75 - gl_gamma.value) / 2.25;
#else
	r = (1.0 - v_gamma.value) / 0.5;
#endif
	M_DrawSlider (220, 60+(OPT_GAMMA*8), r);
	// jkrige - change brightness slider for opengl

	// jkrige - anisotropic filtering
	M_Print (16, 60+(OPT_TEXTUREMODE*8), "          Texture Mode");
#ifdef GLQUAKE
	if (gl_texturemode.value == 0.0f)
		M_Print (220, 60+(OPT_TEXTUREMODE*8), "Point");
	else if (gl_texturemode.value == 1.0f)
		M_Print (220, 60+(OPT_TEXTUREMODE*8), "Bilinear");
	else if (gl_texturemode.value == 2.0f)
		M_Print (220, 60+(OPT_TEXTUREMODE*8), "Trilinear");
	else
	{
		if (modelist[(int)vid_mode.value].width > 640)
			M_Print (220, 60+(OPT_TEXTUREMODE*8), va("Anisotropic %.0fx", pow(2.0f, gl_texturemode.value - 2.0f)));
		else
			M_Print (220, 60+(OPT_TEXTUREMODE*8), va("AF %.0fx", pow(2.0f, gl_texturemode.value - 2.0f)));
	}
#else
	M_Print (220, 60+(OPT_TEXTUREMODE*8), "Point");
#endif
	...
	M_Print (16, 60+(OPT_MOUSESPEED*8), "           Mouse Speed");
	r = (sensitivity.value - 1)/10;
	M_DrawSlider (220, 60+(OPT_MOUSESPEED*8), r);

	M_Print (16, 60+(OPT_MUSICMIDI*8), "            Midi Music");
	M_DrawCheckbox(220, 60+(OPT_MUSICMIDI*8), bgmmidi.value);

	M_Print (16, 60+(OPT_MUSICVOL*8), "          Music Volume");
	r = bgmvolume.value;
	M_DrawSlider (220, 60+(OPT_MUSICVOL*8), r);

	M_Print (16, 60+(OPT_SNDVOL*8), "          Sound Volume");
	r = volume.value;
	M_DrawSlider (220, 60+(OPT_SNDVOL*8), r);

	M_Print (16, 60+(OPT_ALWAYRUN*8),	"            Always Run");
	M_DrawCheckbox (220, 60+(OPT_ALWAYRUN*8), cl_forwardspeed.value > 200);
	...
	M_Print (16, 60+(OPT_CROSSHAIR*8),	"             Crosshair");
...
void M_Quit_Draw (void)
{
...
#ifdef UQE_FMOD
			FMOD_MusicStart ("12", false, false);
#else
	...
	M_PrintWhite (16, y,  "        www.jacqueskrige.com        ");	y += 16;
	// jkrige - hexen 2 credits
